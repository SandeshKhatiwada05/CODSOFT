package org.JSP;

public class CurrencyLists {

    public static void lists(){
        System.out.println("🌍 Common Currency Codes:");
        System.out.println("United States         - USD");
        System.out.println("Nepal                 - NPR");
        System.out.println("India                 - INR");
        System.out.println("European Union (Euro) - EUR");
        System.out.println("United Kingdom        - GBP");
        System.out.println("Japan                 - JPY");
        System.out.println("China                 - CNY");
        System.out.println("South Korea           - KRW");
        System.out.println("Canada                - CAD");
        System.out.println("Australia             - AUD");
        System.out.println("New Zealand           - NZD");
        System.out.println("Switzerland           - CHF");
        System.out.println("Singapore             - SGD");
        System.out.println("Russia                - RUB");
        System.out.println("Thailand              - THB");
        System.out.println("UAE                   - AED");
        System.out.println("Saudi Arabia          - SAR");
        System.out.println("Pakistan              - PKR");
        System.out.println("Bangladesh            - BDT");
        System.out.println("Indonesia             - IDR");
        System.out.println("Malaysia              - MYR");
        System.out.println("South Africa          - ZAR");

    }
}
